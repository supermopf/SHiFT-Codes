...
if(GM_getValue("ShiftMode") === undefined){
	...
	$('div.shift-secondary-title > h1').html("ShiftCode "+(GM_getValue("ShiftIndex")+1)+" von "+ShiftCodes.length)
	if(GM_getValue("ShiftIndex") <= ShiftCodes.length-1 && (($(".alert.notice").val() == undefined)|| $('.alert.notice').html().trim() != "Um weitere SHiFT-Codes einzulösen, starte bitte zuerst ein SHiFT-verbundenes Spiel.")){
		...
		if (GM_getValue("ShiftIndex") > ShiftCodes.length-1 ){
			$('div.shift-secondary-title > h1').html(ShiftCodes.length+" ShiftCodes eingelöst")
			GM_deleteValue("ShiftIndex");
			GM_deleteValue("ShiftMode");
		}else if ($(".alert.notice").val() != undefined && $('.alert.notice').html().trim() == "Um weitere SHiFT-Codes einzulösen, starte bitte zuerst ein SHiFT-verbundenes Spiel."){
			GM_setValue('ShiftIndex',Number(GM_getValue("ShiftIndex"))-1);
