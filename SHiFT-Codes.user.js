...
$('div.main.sh_code_redemption_section > div.shift-secondary-title').before('<div class="btn-group"><button type="button" id="BO1Click" class="btn btn-default">Borderlands 1</button><button type="button" id="BO2Click" class="btn btn-default">Borderlands 2</button><button type="button" id="PSClick" class="btn btn-default">Pre-Sequel</button><button type="button" id="ResetClick" class="btn btn-danger">Reset</button></div>')
...
if(GM_getValue("ShiftMode") == "BO1"){
	ShiftCodes = ['SFCJT-RF6XX-CJ6KB-JJJJ3-6JB33','965BT-56RF6-CJ65B-B3BJJ-CXW39','HFCJT-B6RRR-5BXKT-3BB3J-JCRH5','9XK3B-9W6R6-W3XCB-TJJ33-XFSJS','ZFCB3-6C66R-WJX5B-JBBBT-HRWBS','SRCBJ-5KXFF-5T6K3-33BBB-WBF6H','WJCBB-5RRJK-9B3RJ-6XT3T-CBZBF'];
}



var MsgShiftGame = ['Um weitere SHiFT-Codes einzulösen, starte bitte zuerst ein SHiFT-verbundenes Spiel.','To continue to redeem SHiFT codes, please launch a SHiFT-enabled title first!']
var ContinueRedeemFlag = true

//MsgShiftGame.includes($('.alert.notice').html().trim())


if(GM_getValue("ShiftMode") === undefined){
	//Nothing happend
}else{
	$('div.shift-secondary-title > h1').html("SHiFT-Code "+(GM_getValue("ShiftIndex")+1)+" of "+ShiftCodes.length)


	if($(".alert.notice").val() != undefined){
		//Alert exists
		if(MsgShiftGame.includes($('.alert.notice').html().trim())){
			//Start SHiFT Game
	...
	if(GM_getValue("ShiftIndex") <= ShiftCodes.length-1 && ContinueRedeemFlag){
		...
		setTimeout(function() {
			if($('.redeem_button').length){
				...
				alert('We are too quick... Please wait a minute and redeem this code manually.');
			}
		}, 500);
		GM_setValue('ShiftIndex',Number(GM_getValue("ShiftIndex"))+1);
	}else{
		if (GM_getValue("ShiftIndex") > ShiftCodes.length-1 ){
			$('div.shift-secondary-title > h1').html(ShiftCodes.length+" SHiFT-Codes redeemed")
			GM_deleteValue("ShiftIndex");
			GM_deleteValue("ShiftMode");
		}
	}
